class CalendarCard extends HTMLElement {
  ...
  async getAllEvents(entities) {
    if(!this.lastUpdate || moment().diff(this.lastUpdate, 'minutes') > 
15) {
      const start = 
moment().startOf('day').format("YYYY-MM-DDTHH:mm:ss");
      const end = moment().startOf('day').add(7, 
'days').format("YYYY-MM-DDTHH:mm:ss");

      let urls = entities.map(entity => 
`calendars/${entity}?start=${start}Z&end=${end}Z`);
      ...
      if(this.config.showProgressBar) {
        if(events.length > 0 && moment().format('DD') === 
moment(events[0].startDateTime).format('DD')) {
          let now = {startDateTime: moment().format(), type: 'now'}
          events.push(now);
        }
      }

      events.sort((a, b) => new Date(a.startDateTime) - new 
...
events);
  ...
  getDayHtml(day, events) {
    let clazz = moment().format('DD') === moment(day).format('DD') ? 
'date now' : 'date';
    return `
      <div class="day">
        <div class="${clazz}">
        ...
        <div class="events">${events.map(event => 
this.getEventHtml(event)).join('')}</div>
      </div>`;
  }

  getEventHtml(event) {
    if(event.type) {
        return `<ha-icon icon="mdi:circle" class="now"></ha-icon><hr 
class="now" />`;
    }
    //${event.summary.indexOf('birthday') > 0 ? `<div 
class="congrats"><i class="fas fa-gift"></i>&nbsp;Send congrats</div>` : 
''}
    return `
      <div class="event-wrapper">
        <div class="event">
          <div class="info">
            ...
            ${event.location ? `<div class="location"><ha-icon 
icon="mdi:map-marker"></ha-icon>&nbsp;${event.location}</div>` : ''}
            
          </div>
          <div class="time">${event.isFullDayEvent ? 'All day' : 
(moment(event.startDateTime).format('HH:mm') + `-` + 
moment(event.endDateTime).format('HH:mm'))}</div>
        </div>
      </div>`;
  }
  ...
  setConfig(config) {
    if (!config.entities) {
      throw new Error('You need to define at least one calendar entity 
via entities');
    }
    this.config = {
      name: 'Calendar',
  ...
  isSomethingChanged(events) {
    let isSomethingChanged = JSON.stringify(events) !== 
